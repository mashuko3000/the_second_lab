...
const char *error_messages[5] = {
    ...
    "Memory allocation error",     // ERROR_MEMORY
    "File open error",             // ERROR_OPEN_FILE
    "Error freeing memory",        // ERROR_FREE_MEMORY
    "Unknown action error"         // XZ_CHO_DELAT
};
...
int release_resources(void *first_resource, ...) {
    ...
    while (resource != NULL) {
        ...
        // Обрабатываем действия без switch-case
